...
def Shifted_Rotated_High_Conditioned_Elliptic(x, M, o):
    M = np.ascontiguousarray(M)
    return High_Conditioned_Elliptic(np.dot(M, x - o[0:x.shape[0]])) + 100


@numba.cfunc("f8(f8[:],f8[:,:],f8[:])", nogil=True, fastmath=True)
def f1(x, M, o):
    return Shifted_Rotated_High_Conditioned_Elliptic(x, M, o)
...
def Shifted_Rotated_BentCigar(x, M, o):
    M = np.ascontiguousarray(M)
    return Bent_Cigar(np.dot(M, x - o[0:x.shape[0]])) + 200
...
def Shifted_Rotated_Discus(x, M, o):
    M = np.ascontiguousarray(M)
    return Discus(np.dot(M, x - o[0:x.shape[0]])) + 300
...
def Shifted_Rotated_Ackley(x, M, o):
    M = np.ascontiguousarray(M)
    return Ackley(np.dot(M, x - o[0:x.shape[0]])) + 500


@numba.cfunc("f8(f8[:],f8[:,:],f8[:])", nogil=True, fastmath=True)
def f5(x, M, o):
    return Shifted_Rotated_Ackley(x, M, o)
...
def Shifted_Rotated_Weierstrass(x, M, o):
    M = np.ascontiguousarray(M)
    return Weierstrass(np.dot(M, 0.005 * (x - o[0:x.shape[0]]))) + 600
...
def Shifted_Rotated_Griewank(x, M, o):
    M = np.ascontiguousarray(M)
    return Griewank(np.dot(M, 6 * (x - o[0:x.shape[0]]))) + 700


@numba.cfunc("f8(f8[:],f8[:,:],f8[:])", nogil=True, fastmath=True)
def f7(x, M, o):
    return Shifted_Rotated_Griewank(x, M, o)
...
def Shifted_Rastrigin(x, o):
    return Rastrigin(0.0512 * (x - o[0:x.shape[0]])) + 800


@numba.cfunc("f8(f8[:],f8[:,:],f8[:])", nogil=True, fastmath=True)
def f8(x, M, o):
    return Shifted_Rastrigin(x, o)


@numba.njit("f8(f8[:],f8[:,:],f8[:])", nogil=True, fastmath=True)
def Shifted_Rotated_Rastrigin(x, M, o):
    M = np.ascontiguousarray(M)
    return Rastrigin(np.dot(M, 0.0512 * (x - o[0:x.shape[0]]))) + 900
...
def Shifted_Schwefel(x, o):
    return Schwefel(10 * (x - o[0:x.shape[0]])) + 1000
...
def Shifted_Rotated_Katsuura(x, M, o):
    M = np.ascontiguousarray(M)
    return Katsuura(np.dot(M, 0.05 * (x - o[0:x.shape[0]]))) + 1200
...
def Shifted_Rotated_HappyCat(x, M, o):
    M = np.ascontiguousarray(M)
    return HappyCat(np.dot(M, 0.05 * (x - o[0:x.shape[0]]))) + 1300
...
def Shifted_Rotated_HGBat(x, M, o):
    M = np.ascontiguousarray(M)
    return HGBat(np.dot(M, 0.05 * (x - o[0:x.shape[0]]))) + 1400
...
def Shifted_Rotated_Expended_Griewank_plus_Rosenbrock(x, M, o):
    M = np.ascontiguousarray(M)
    return Expended_Griewank_plus_Rosenbrock(np.dot(M, 0.05 * (x - o[0:x.shape[0]]))) + 1500
...
def Shifted_Rotated_Expanded_Scaffer_F6(x, M, o):
    M = np.ascontiguousarray(M)
    return Expanded_Scaffer_F6(np.dot(M, x - o[0:x.shape[0]])) + 1600
...
def Hybrid_2(x, M, o, s):
    ...
    fit += HGBat(0.05 * x[0.3*dim:0.6*dim])
...
def Hybrid_4(x, M, o, s):
    ...
    x = np.dot(np.ascontiguousarray(M), x - o[0:dim])
    x = x[s]
    fit = HGBat(0.05 * x[0:0.2*dim])
    fit += Discus(x[0.2*dim:0.4*dim])
    fit += Expended_Griewank_plus_Rosenbrock(0.05 * x[0.4*dim:0.7*dim])
...
def Composition_2(x, M, o):
    ...
    fit += w[2] * (HGBat(np.dot(np.ascontiguousarray(M[2*dim:3*dim, 0:dim]), 0.05 * (x - o[2*dim:3*dim]))) + 200)
    return fit + 2400
...
def Composition_4(x, M, o):
    ...
    fit += w[2] * (High_Conditioned_Elliptic(np.dot(np.ascontiguousarray(M[2*dim:3*dim, 0:dim]), x - o[2*dim:3*dim])) * 1e-7 + 200)
    fit += w[3] * (Weierstrass(np.dot(np.ascontiguousarray(M[3*dim:4*dim, 0:dim]), 0.005 * (x - o[3*dim:4*dim]))) * 2.5 + 300)
    fit += w[4] * (Griewank(np.dot(np.ascontiguousarray(M[4*dim:5*dim, 0:dim]), 6 * (x - o[4*dim:5*dim]))) * 10 + 400)
    return fit + 2600
...
def Composition_6(x, M, o):
    ...
    delta = np.array([10, 20, 30, 40, 50])
    w = composition_omega(x, o, np.square(delta))
    fit = w[0] * Expended_Griewank_plus_Rosenbrock(np.dot(np.ascontiguousarray(M[0:dim, 0:dim]), 0.05 * (x - o[0:dim]))) * 2.5
    fit += w[1] * (HappyCat(np.dot(np.ascontiguousarray(M[dim:2*dim, 0:dim]), 0.05 * (x - o[dim:2*dim]))) * 10 + 100)
    fit += w[2] * (Schwefel(np.dot(np.ascontiguousarray(M[2*dim:3*dim, 0:dim]), 10 * (x - o[2*dim:3*dim]))) * 2.5 + 200)
    fit += w[3] * (Expanded_Scaffer_F6(np.dot(np.ascontiguousarray(M[3*dim:4*dim, 0:dim]), 0.05 * (x - o[3*dim:4*dim]))) * 1e+4 / 2e+7 + 300)
    fit += w[4] * (High_Conditioned_Elliptic(np.dot(np.ascontiguousarray(M[4*dim:5*dim, 0:dim]), x - o[4*dim:5*dim])) * 1e-6 + 400)
    return fit + 2800
...
@numba.njit("f8(f8[:],f8[:,:],f8[:],int32[:])", nogil=True, fastmath=True)
def Composition_7(x, M, o, s):
    dim = x.shape[0]
    delta = np.array([10, 30, 50])
    w = composition_omega(x, o, np.square(delta))
    fit = w[0] * Hybrid_1(x, M[0:dim, 0:dim], o[0:dim], s[0:dim])
    fit += w[1] * (Hybrid_2(x, M[dim:2*dim, 0:dim], o[dim:2*dim], s[dim:2*dim]) + 100)
    fit += w[2] * (Hybrid_3(x, M[2*dim:3*dim, 0:dim], o[2*dim:3*dim], s[2*dim:3*dim]) + 200)
...
@numba.cfunc("f8(f8[:],f8[:,:],f8[:],int32[:])", nogil=True, fastmath=True)
def f29(x, M, o, s):
    return Composition_7(x, M, o, s)


@numba.njit("f8(f8[:],f8[:,:],f8[:],int32[:])", nogil=True, fastmath=True)
def Composition_8(x, M, o, s):
    dim = x.shape[0]
    delta = np.array([10, 30, 50])
    w = composition_omega(x, o, np.square(delta))
    fit = w[0] * Hybrid_4(x, M[0:dim, 0:dim], o[0:dim], s[0:dim])
    fit += w[1] * (Hybrid_5(x, M[dim:2*dim, 0:dim], o[dim:2*dim], s[dim:2*dim]) + 100)
    fit += w[2] * (Hybrid_6(x, M[2*dim:3*dim, 0:dim], o[2*dim:3*dim], s[2*dim:3*dim]) + 200)
